...
(function(global, factory) {
					...
					while ((elem = elem[dir])) {
						if (elem.nodeType === 1 || checkNonElements) {
							...
							if ((oldCache = outerCache[dir]) && oldCache[0] === dirruns && oldCache[1] === doneName) {
								...
								// Assign to newCache so results back-propagate
								...
								// A match means we're done; a fail means we
	...
	function actualDisplay(name, doc) {
		...
		// Use of this method is a temporary fix (more like optmization) until
	...
	jQuery.parseJSON = function(data) {
		...
		return str && !jQuery.trim(str.replace(rvalidtokens, function(token, comma, open, close) {
			...
			// array/object close ("]" or "}"): depth += false - true
			// (decrement)
			// other cases ("," or primitive): depth += true - true (numeric
			// cast)
			depth += !close - !open;
